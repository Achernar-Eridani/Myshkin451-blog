    ...
    <div class="comments-manager">
      ...
      <div class="bg-white shadow rounded-lg p-6">
        ...
        <div v-else class="space-y-6">
          <div v-for="comment in comments" :key="comment.id" class="border-b last:border-b-0 pb-6 mb-6 last:pb-0 last:mb-0">
            <div class="flex items-start">
              <img :src="comment.user?.avatar || '/img/default-avatar.png'" alt="用户头像" class="w-10 h-10 rounded-full mr-3">
              <div class="flex-1">
                <div class="flex flex-wrap justify-between items-start">
                  <div>
                    <span class="font-medium text-gray-900">{{ comment.user?.username || '未知用户' }}</span>
                ...
                <div v-if="comment.replies && comment.replies.length > 0" class="mt-4 pl-4 border-l-2 border-gray-200">
                  <div v-for="reply in comment.replies" :key="reply.id" class="mb-3 last:mb-0">
                    <div class="flex items-start">
                      <img :src="reply.user?.avatar || '/img/default-avatar.png'" alt="回复用户头像" class="w-8 h-8 rounded-full mr-2">
                      <div>
                        <div class="flex items-center flex-wrap">
                          <span class="font-medium text-gray-800">{{ reply.user?.username || '未知用户' }}</span>
