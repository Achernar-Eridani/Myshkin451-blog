          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="post in posts" :key="post.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <button @click="$emit('edit', post)" class="text-blue-600 hover:text-blue-900 mr-3">编辑</button>
                <a :href="`/posts/${post.id}`" target="_blank" class="text-green-600 hover:text-green-900">查看</a>
              </td>
            </tr>
          </tbody>
        </table>
